...
- (ThingMO *) createThingMO {
    NSManagedObjectContext *moc = self.persistentContainer.viewContext;
    ThingMO *thingMO = [NSEntityDescription insertNewObjectForEntityForName:@"Thing" inManagedObjectContext:moc];
    NSLog(@"createThingMO: %@", thingMO);
    return thingMO;
}
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"PeerAssignment4"];
